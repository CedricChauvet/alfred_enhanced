...
class Module(Base):
    ...
    def __init__(self, args, vocab):
        ...
        #
        # Décodeur d'actions bas niveau
        # Ce décodeur prend les représentations visuelles et linguistiques enrichies
        # pour générer séquentiellement les actions à effectuer
        decoder = vnn.ConvFrameMaskDecoderProgressMonitor if self.subgoal_monitoring else vnn.ConvFrameMaskDecoder

        self.dec = decoder(
            self.emb_action_low,           # Embedding des actions bas niveau (ex: MoveAhead, RotateLeft, PickupObject)
            args.dframe,                   # Dimension des features visuelles extraites des frames (ex: 2048 pour ResNet)
            2*args.dhid,                   # Dimension du contexte linguistique = 2×dhid car LSTM bidirectionnel
                                        # (forward + backward concaténés)
            pframe=args.pframe,            # Nombre de frames visuelles à traiter en parallèle (masque pyramidal)
            attn_dropout=args.attn_dropout,      # Dropout pour le mécanisme d'attention visuelle
            hstate_dropout=args.hstate_dropout,  # Dropout sur les états cachés du LSTM décodeur
            actor_dropout=args.actor_dropout,    # Dropout avant la couche de classification d'actions
            input_dropout=args.input_dropout,    # Dropout sur les inputs (embeddings + features)
            teacher_forcing=args.dec_teacher_forcing  # Si True: utilise les vraies actions pendant l'entraînement
                                                    # Si False: utilise ses propres prédictions (autoregressive)
        )
        # dropouts
    ...
    def encode_lang(self, feat):
        ...
        emb_lang_goal_instr = feat['lang_goal_instr']  # PackedSequence
        
        # Dropout (régularisation)
        self.lang_dropout(emb_lang_goal_instr.data)
        
        # LSTM bidirectionnel (CŒUR de l'encodage)
        enc_lang_goal_instr, _ = self.enc(emb_lang_goal_instr)
        # Forward: →→→ + Backward: ←←← = 256 dimensions
        
        # Dépacker (PackedSequence → Tensor)
        ...
        # Self-Attention (résumer en UN vecteur)
        ...
        ### **Output 1 : cont_lang** (batch, 256)
        """
        🎯 UN vecteur résumé par exemple
        📦 Résume TOUTE l'instruction en un seul vecteur
        💡 Capture l'intention globale

        Utilisation :
        ├─ Initialise l'état h_0 du decoder (baseline)
        └─ Initialise l'état du subgoal_decoder (CoT)

        Analogie : Le "titre" du livre


        ### **Output 2 : enc_lang** (batch, seq_len, 256)
        
        🎯 UN vecteur PAR TOKEN
        📦 Préserve tous les détails
        💡 Le decoder peut "regarder" chaque mot

        Utilisation :
        ├─ Attention dans le decoder (baseline)
        └─ Enrichi avec subgoals (CoT)

        Analogie : Le "texte complet" du livre
        """
    ...
    def step(self, feat, prev_action=None):
        ...
        '''
        # ✓✓✓ FIX GPU: forcer frames sur device ✓✓✓
        device = next(self.parameters()).device
        if 'frames' in feat and isinstance(feat['frames'], torch.Tensor):
            feat['frames'] = feat['frames'].to(device)
    ...
    def compute_loss(self, out, batch, feat):
        ...
        '''
        # ✓✓✓ FIX GPU: déplacer tenseurs vers device ✓✓✓
        device = next(self.parameters()).device
    ...
    def weighted_mask_loss(self, pred_masks, gt_masks):
        '''
        mask loss that accounts for weight-balance between 0 and 1 pixels
        '''
        # ✓✓✓ FIX GPU ✓✓✓
        device = next(self.parameters()).device
